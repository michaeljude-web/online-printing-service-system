...
def approve_customer(customer_id, refresh_func):
    try:
        mydb = db_connection()
        cursor = mydb.cursor()
        cursor.execute("UPDATE customer SET status = 'Approved' WHERE customer_id = %s", (customer_id,))
        mydb.commit()
...
def customer(parent):
    ...
    try:
        mydb = db_connection()
        cursor = mydb.cursor()
        cursor.execute("SELECT customer_id, full_name, email FROM customer WHERE status = 'Pending'")
    ...
    if not pending_customers:
        Label(parent, text="No pending customers.", font=("Arial", 10), bg="white").pack(padx=20, pady=20)
        return
    ...
    for customer_data in pending_customers:
        ...
        Label(info, text=full_name,
              font=("Arial", 10, "bold"),
              bg="white", fg="black").pack(anchor="w")

        Label(info, text=email,
              font=("Arial", 9),
              bg="white", fg="black").pack(anchor="w")
        ...
        Button(buttons,
               ...
               font=("Arial", 9),
               width=10,
               command=lambda cid=customer_id: approve_customer(cid, lambda: customer(parent))
        ...
        Button(buttons,
               ...
               font=("Arial", 9),
               width=3,
               command=lambda cid=customer_id: delete_customer(cid, lambda: customer(parent))
               ).pack(side="left", padx=5)
